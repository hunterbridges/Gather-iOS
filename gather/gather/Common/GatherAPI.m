...
+ (void) request:(NSString *)apiMethod requestMethod:(NSString *)requestMethod requestData:(NSMutableDictionary *)requestData
{
    ...
    {
        [requestData setObject:[[SessionData sharedSessionData] token] forKey:@"token"];
        [requestData setObject:[[SessionData sharedSessionData] verification] forKey:@"verification"];
    }
	...
	if ([requestMethod isEqualToString:@"GET"])
	{
		...
		NSURL * assembledURL = [[NSURL alloc] initWithString:[NSString stringWithFormat:@"%@%@?%@",apiUrl,apiMethod,queryString]];
		NSLog(@"%@",assembledURL);
		
		req = [[NSMutableURLRequest alloc] initWithURL:assembledURL];
		
		[assembledURL release];
	} else if ([requestMethod isEqualToString:@"POST"] || [requestMethod isEqualToString:@"DELETE"] || [requestMethod isEqualToString:@"PUT"])
	{
		NSURL * assembledURL = [[NSURL alloc] initWithString:[NSString stringWithFormat:@"%@%@",apiUrl,apiMethod]];
		NSString * boundary = @"0xKhTmLbOuNdArY";
		
		NSMutableData * requestBody = [[self dictionaryToMultipartEncodedData:requestData boundary:boundary] retain];
		req = [[NSMutableURLRequest alloc] initWithURL:assembledURL];
		[req setHTTPMethod:@"POST"];
		[req setValue:[NSString stringWithFormat:@"multipart/form-data; boundary=%@",boundary] forHTTPHeaderField:@"Content-Type"];
		[req setHTTPBody:requestBody];
		[req setValue:[NSString stringWithFormat:@"%d",[requestBody length]] forHTTPHeaderField:@"Content-Length"];
	}
